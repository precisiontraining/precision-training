...
export default function ExerciseSearchInput({ value, onChange, placeholder = 'Search exercises…' }) {
  ...
  function handleInput(e) {
    ...
    setQuery(val)
    // Pass the typed value directly so the parent always has the current text.
    // An empty string is treated as "nothing entered" by the parent.
    ...
    <div className={styles.wrap} ref={wrapRef}>
      ...
      {query.trim().length >= 2 && !isOpen && results.length === 0 && (
        <div className={styles.noResults}>
          No match found — the exercise will be added without a preview image.
        </div>
      )}
    </div>
  )
