...
// ── EXERCISE IMAGE with error fallback ───────────────────────────────────────
// Renders the gif if src is available, falls back to ExerciseFallback on error.
function ExerciseImage({ src, name, imgClass, wrapClass }) {
  const [failed, setFailed] = useState(false)
  // Reset failed state when src changes (e.g. after a swap)
  ...
  // No src or failed to load → show nothing (no placeholder needed since caller handles layout)
  return wrapClass
    ? <div className={wrapClass}><ExerciseFallback name={name} /></div>
    : <ExerciseFallback name={name} />
}
...
export default function PlanView() {
  ...
  async function fetchImages(html, parsed) {
    ...
    // First: resolve images from the local exerciseList (instant, no network needed)
    const { EXERCISE_LIST } = await import('../utils/exerciseList.js')
    const localMap = {}
    EXERCISE_LIST.forEach(ex => {
      if (ex.gifUrl) localMap[ex.name.toLowerCase().trim()] = ex.gifUrl
    })

    // Seed state with any local matches immediately
    const localHits = {}
    names.forEach(name => {
      if (localMap[name]) { localHits[name] = localMap[name]; return }
      // Fuzzy: normalise plurals, dashes, underscores
      const norm = name.replace(/s$/, '').replace(/[-_]/g, ' ').replace(/\s+/g, ' ')
      const hit = Object.entries(localMap).find(([k]) => {
        const kn = k.replace(/s$/, '').replace(/[-_]/g, ' ')
        return kn === norm || k.includes(norm) || norm.includes(k)
      })
      if (hit) localHits[name] = hit[1]
    })
    if (Object.keys(localHits).length > 0) {
      setImages(prev => ({ ...prev, ...localHits }))
    }

    // Then: fetch remaining names from Supabase edge function in parallel
    const missing = [...names].filter(name => !localHits[name])
              ...
              return (
                <div key={i} className={styles.exerciseCard} data-tour={i===0 ? 'exercise-card' : undefined}>
                  ...
                  <div className={styles.exerciseImg}>
                    <ExerciseImage src={img} name={ex.name} imgClass={styles.exImg} wrapClass={styles.exImgPlaceholder} />
                  </div>
              ...
              return (
                <div key={i} className={styles.exerciseCardGrid} data-tour={i===0 ? 'exercise-card' : undefined}>
                  <div className={styles.exerciseImgGrid}>
                    <ExerciseImage src={img} name={ex.name} imgClass={styles.exImg} />
                  </div>
